...
options(scipen=999)
df.1 <- read.csv("3egGdVzYlecw4Q4F7CIUzU.csv") #Best of Joaquin Sabina
df.2 <- read.csv("5zQZYAR0pJ0aJBaaIPvP3T.csv") #Best of The Beatles

# NOTE: Everything df.1 is Joaquín Sabina, and df.2 is The Beatles
...
ggplot(df.1.i, aes(x = reorder(feature, -mean), y = mean)) +
  ...
  ggtitle("Mean value of audio features of Joaquín Sabina music playlist") +
  xlab("Features") +
  ylab("Mean") +
...
ggplot(df.2.i, aes(x = reorder(feature, -mean), y = mean)) +
  ...
  theme(axis.text = element_text(colour = 'black')) +
  ggtitle("Mean value of audio features of The Beatles music playlist") +
  xlab("Features") +
  ylab("Mean") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
df.mean.difference$who <- ifelse(df.mean.difference$difference > 0, 'Joaquin Sabina', 'The Beatles')

ggplot(df.mean.difference, aes(x = reorder(feature, -difference), 
                               y = difference, fill = who)) +
  geom_bar(stat='identity') +
  ggtitle("Difference between audio features mean of Joaquin Sabina and The Beatles music") +
  xlab("Feature") +
  ylab("Difference") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
ggplot(df.1.i, aes(x = reorder(feature, -sd), y = sd)) +
  geom_bar(stat='identity', fill = '#00BFC4') +
  theme(axis.text = element_text(colour = 'black')) +
  ggtitle("Standard deviation of the audio features scores of Joaquin Sabina music playlist") +
  xlab("Feature")+
...
ggplot(df.2.i, aes(x = reorder(feature, -sd), y = sd)) +
  geom_bar(stat='identity', fill = '#F8766D') +
  theme(axis.text = element_text(colour = 'black')) +
  ggtitle("Standard deviation of the audio features scores of The Beatles music playlist") +
  xlab("Feature")+
  ylab("Standard Deviation") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
ggplot(df.1.long, aes(factor(variable), value)) + geom_boxplot() + ggtitle("Boxplot for features of Joaquín Sabina music playlist")
...
ggplot(df.2.long, aes(factor(variable), value)) + geom_boxplot() + ggtitle("Boxplot for features of The Beatles music playlist")
...
ggplot(df.1, aes(x = energy, y = danceability)) +
  ...
  ggtitle("Correlation between danceability and energy (Joaquín Sabina)")

ggplot(df.1, aes(x = energy, y = acousticness)) +
  geom_point() +
  geom_smooth(method = "loess", se = FALSE) +
  ggtitle("Correlation between acousticness and energy (Joaquín Sabina)")
...
ggplot(df.2, aes(x = energy, y = valence)) +
  geom_point() +
  geom_smooth(method = "loess", se = FALSE) +
  ggtitle("Correlation between valence and energy (The Beatles)")

ggplot(df.2, aes(x = energy, y = acousticness)) +
  geom_point() +
  geom_smooth(method = "loess", se = FALSE) +
  ggtitle("Correlation between acousticness and energy (The Beatles)")
...
boring.1 <- data.frame(boringness = boringness(df.1), uri = df.1$uri, who = 'Joaquín Sabina')
...
ggplot(boring.1, aes(boringness)) +
  geom_histogram() +
  ggtitle("Histogram of the boringness score (Joaquín Sabina)")

boring.2 <- data.frame(boringness = boringness(df.2), uri = df.2$uri, who = 'The Beatles')

ggplot(boring.2, aes(boringness)) +
  geom_histogram() +
  ggtitle("Histogram of the boringness score (The Beatles)")
...
boring.stats <- data.frame(mean = c(mean(boring.1$boringness), mean(boring.2$boringness)),
                           ...
                           who = c('Joaquín Sabina', 'The Beatles'))
...
implicit.1$who <- 'Joaquín Sabina'
implicit.2$who <- 'The Beatles'
